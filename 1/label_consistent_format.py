...
root_directory = '/Users/schnappiiii/Applied_Machine_Intelligence/project'
...
label_dict = {'1': 'Dent', '2': 'Other', '3': 'Rim', '4': 'Scratch'}   # dictionary{label:name} 
save_label = pd.DataFrame(columns=['annotation_index', 'image_id', 'human_label', 'label_name'])  # build empty dataframe

tmp = data_bbox.iloc[720:900,:]  # the number of images that you want to label
for i,name in tmp.iterrows():
    ...
    print('1: Dent, 2: Other, 3: Rim, 4: Scratch')
    ...
    save_label.loc[i] = [i, image_id, label, label_dict[str(label)]]
